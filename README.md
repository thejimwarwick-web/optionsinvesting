...
`portfolio-collect` never accepts an operator-authored replacement snapshot. It verifies each content-addressed/sealed record and its ordered predecessor chain, then deterministically replays append-only ledger records for initialization, cash, positions, marks, valuation comparisons, and pending-submission/resolution events; NAV is recomputed from cash, marked holdings, option liabilities, and applicable reservations, while stated valuations are evidence that must agree; `--initialize` is accepted only once against an empty ledger and creates the £100,000 bootstrap. The sealed snapshot retains every source record ID and may be compared with `--expected-state` for reconciliation.
